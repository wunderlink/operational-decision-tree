  ...
  if (node.decisions) {
    async.forEachOf(node.decisions, function (decision, index, callback) {
      if (resolved) {
        ...
        self.prepareSubject(nodeCount, node, decision, subjectData, populated, function (err) {
          if (err) return cb(err)
          var result = self.runComparisons(nodeCount, decision, subjectData, populated)
          if (result) {
            resolved = true
          } else {
            resolvedIndex++
...
DecisionTree.prototype.prepareSubject = function (nodeCount, node, comparisons, subjectData, populated, cb) {
  var self = this
  async.each(comparisons, function (comparison, callback) {
    if (populated && populated[nodeCount] && populated[nodeCount][comparison.property]) {
      callback(null)
...
DecisionTree.prototype.compare = function (nodeCount, property, comparison, subjectData, populated) {
  var result = 0;
  if (populated && populated[nodeCount] && populated[nodeCount][property]) {
