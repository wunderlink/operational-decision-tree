...
DecisionTree.prototype.testCondition = function (condition, subjectData, subjectPriority, cb) {
  try {
    var result = this.runComparisons(condition, subjectData, subjectPriority)
  } catch (err) {
    return cb(err)
  }
  if (this.opts.conditionOpts[condition.name] && this.opts.conditionOpts[condition.name].hit) {
    this.opts.conditionOpts[condition.name].hit(subjectData, function (err) {
      if (err) return cb(err)
...
DecisionTree.prototype.runComparisons = function (condition, subjectData, subjectPriority) {
  var run = null
  if (this.opts.conditionOpts[condition.name] && this.opts.conditionOpts[condition.name].run) {
