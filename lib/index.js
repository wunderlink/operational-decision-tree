...
DecisionTree.prototype.runNode = function (nodeCount, node, subjectData, populated, cb) {
  ...
  if (node.makeDecision) {
    // run prepare for each decision then run makeDecision
    async.forEachOf(node.decisions, function (comparisons, index, callback) {
      ...
      if (err) return cb(err)
      if (self.opts.makeDecision[node.makeDecision]) {
        var pop = []
        if (populated && populated[nodeCount]) {
          pop = populated[nodeCount]
        }
        var result = self.opts.makeDecision[node.makeDecision](node.decisions, subjectData, pop)
      } else {
        callback('missing makeDecision function: '+node.makeDecision)
      }
    })
    ...
    async.forEachOf(node.decisions, function (comparisons, index, callback) {
      if (resolved) {
        ...
        self.prepareSubject(nodeCount, node, node.decisions, comparisons, subjectData, populated, function (err) {
          if (err) return cb(err)
          var result = self.runDecisions(nodeCount, comparisons, subjectData, populated)
          if (result) {
            resolved = true
          } else {
            resolvedIndex++
...
DecisionTree.prototype.runDecisions = function (nodeCount, comparisons, subjectData, populated) {
  var self = this
